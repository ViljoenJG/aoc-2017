...
Program.prototype.getNestedWeight = function() {
  return this.programs.reduce((sum, x) =>
    sum + x.getNodeWeight(), 0);
};

Program.prototype.getNodeWeight = function() {
  return this.getNestedWeight() + this.weight;
};

Program.prototype.isNodeBalanced = function() {
  if (this.programs.length === 0) return true;
  var programWeights = this.programs.map(x => x.getNodeWeight())

  for (var a=1; a<programWeights.length; a++) {
    if (programWeights[a] !== programWeights[0]) {
      return false;
    }
  }
...
Program.prototype.isChildrenBalanced = function() {
  return !!this.getUnbalancedChild();
};

Program.prototype.getUnbalancedChild = function() {
  if (this.programs.length === 0) return null;
  return this.programs.reduce((acc, x) => {
    if (acc) return acc;
    return x.isNodeBalanced() ? null : x;
  }, null);
}

function buildSubTree(node) {
  if (node.constructor !== Program) {
    node = programMap[node];
  }

  node.programs = node.programs.map(child => buildSubTree(child));
  return node;
}
...
var programs = `dihjv (2158) -> gausx, ncdmp, hozgrub
...
var programMap = programs.reduce((map, x) => {
  map[x.name] = x;
  return map;
},{});
...
var rootProgram = programs.filter(x => !nestedParents[x.name])[0];
var tree = buildSubTree(rootProgram);
